...
abstract contract PostChecker_EmergencyExit is BaseMigration, PostChecker_Helper {
  ...
  function _postCheck__RequestEmergencyExit() private logFn("Post check request emergency exit") {
    vm.startPrank(_candidateAdmin);
    // Should request emergency exit success
    (bool success, ) = _staking.call(
      abi.encodeWithSelector(ICandidateStaking.requestEmergencyExit.selector, _consensusAddr)
    );
    assertTrue(success);

    // Should fail to request emergency exit again
    (success, ) = _staking.call(
      abi.encodeWithSelector(ICandidateStaking.requestEmergencyExit.selector, _consensusAddr)
    );
    assertFalse(success);
    vm.stopPrank();
