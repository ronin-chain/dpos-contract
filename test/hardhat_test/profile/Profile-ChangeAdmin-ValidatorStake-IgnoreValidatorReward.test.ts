...
describe('Profile: change admin (staking reward on the changing day is splitted to delegators)', () => {
  before(async () => {
    ...
    } = await deployTestSuite('Profile-ChangeAdmin-ValidatorStake')({
  ...
  describe('Change admin from A0 to A1', async () => {
    ...
    describe('Effects on Staking contract', async () => {
      ...
      it('Should the staking reward cannot be claimed (even by A0 or A1), and splitted to delegators', async () => {
        ...
        await expect(tx).emit(stakingContract, 'RewardClaimed').withArgs(validatorCandidates[0].consensusAddr.address, validatorCandidates[0].poolAdmin.address, 0);
      })
      ...
      it('Should the staking reward of A1 is updated as usual on the day after', async() => {
        await network.provider.send('hardhat_setCoinbase', [validatorCandidates[0].consensusAddr.address]);
        await validatorContract.connect(validatorCandidates[0].consensusAddr).submitBlockReward({value: 10_000});
        ...
        {
          expect(await stakingContract.getReward(validatorCandidates[0].consensusAddr.address, validatorCandidates[0].candidateAdmin.address)).eq(7999);
          let rewards =  await stakingContract.connect(validatorCandidates[0].poolAdmin).getRewards(validatorCandidates[0].poolAdmin.address, [validatorCandidates[0].consensusAddr.address]);
          expect(rewards[0]).eq(7999);

          let tx = await stakingContract.connect(validatorCandidates[0].poolAdmin).claimRewards([validatorCandidates[0].consensusAddr.address]);
          await expect(tx).emit(stakingContract, 'RewardClaimed').withArgs(validatorCandidates[0].consensusAddr.address, validatorCandidates[0].poolAdmin.address, 7999);
        }
      })
    })
  });
