...
contract Maintenance_Concrete_Test is Maintenance_Base_Test {
  function testConcrete_NewAdminChangedInProfile_CanSchedule_schedule() external {
    address[] memory validatorIds = validatorSet.getValidatorCandidateIds();
    address admin = profile.getId2Profile(validatorIds[0]).admin;
    address newAdmin = makeAddr("new-admin");

    vm.prank(admin);
    profile.changeAdminAddr(validatorIds[0], newAdmin);
  ...
  function testConcrete_RevertIf_CandidateRequestRenounce_schedule() external {
    address[] memory cids = validatorSet.getValidatorCandidateIds();

    IProfile.CandidateProfile memory profile = profile.getId2Profile(cids[cids.length - 1]);
    address admin = profile.admin;
    vm.prank(admin);
    staking.requestRenounce(profile.consensus);
    ...
    vm.expectRevert();
    vm.prank(admin);
    maintenance.schedule(profile.consensus, startAt, endAt);
  }
